...
func TestCreateRepoNoError(t *testing.T) {
	...
	restclient.AddMockup(restclient.Mock{
		...
		Response: &http.Response{
			...
			Body:       ioutil.NopCloser(strings.NewReader(`{"id": 123, "name": "testing", "owner": {"login": "miguelhun"}}`)),
		},
	})
	request := repositories.CreateRepoRequest{Name: "testing"}
	...
	assert.EqualValues(t, "miguelhun", result.Owner)
}
...
func TestCreateRepoConcurrentNoError(t *testing.T) {
	...
	assert.EqualValues(t, "miguelhun", result.Response.Owner)
}
...
func TestCreateReposInvalidRequests(t *testing.T) {
	...
	assert.EqualValues(t, "invalid repo name", result.Results[1].Error.Message())
}
